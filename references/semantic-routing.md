...
|---|---|---|---|
| `negative-friction` | still manual, repeated, inefficient, risky, too slow | `redWarningCard` | `highlightBox` |
| `negative-to-positive` | not A but B; pain then spoken resolution | `redWarningToGreenConfirm` | `highlightBox` |
...
| `automation-handoff` | Codex/system takes over the task | `automationHandoff` | `captionHighlight` |
| `platform-fanout` | distribute one package to Douyin, Xiaohongshu, Bilibili, channels | `platformFanout` | `transitionPushZoom` |
| `asset-variants` | horizontal/vertical/square posters, multi-size covers | `ratioGallery` | `flowPath` |
...
The regression suite currently covers 96 spoken examples across the confirmed V1 routes, including a real project Hook sequence guard that prevents repeated big-title routing. The SFX regression suite covers the six confirmed semantic audio suggestions and requires `status: "suggested"`. A route change should update the test case and this document in the same commit.
