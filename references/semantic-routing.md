...
- Result-evaluation polarity is equally mandatory. Asserted `结果正确`, `验证通过`, `执行成功`, `没有错误`, `失败项为0`, or equivalent zero-error wording routes to `positive-confirm`; asserted `结果不正确`, `执行失败`, `这一步出错`, `验证没有通过`, or equivalent failed status routes to `negative-friction`. Questions (`是否正确`), possibility (`可能出错`), prevention (`避免错误`), conditional/prospective wording, and noun/meta uses (`成功按钮` / `错误示例`) do not trigger result SFX. When multiple asserted evaluations occur, the latest scoped result wins.
...
- Arabic and Chinese quantities share one source-faithful numeric parser. `十张详情图还没生成完` must produce the same numeric value/suffix and incomplete treatment as `10张详情图还没生成完`; never replace an unparsed source number with a gallery default.
- A handoff is separate from completion. `把素材交给 Codex 自动完成` is `automation-handoff` with processing treatment; `Codex 还没有接管这一步` is `negative-friction`; `交给 Codex 之前，先检查素材` is a prerequisite `workflow-step`, not a handoff.
- Three or more same-scene cues explicitly ordered by `首先/第一步/先` → `然后/接着/其次/再` → `最后/最终/第三步` form one source-bound `workflow-step`. Keep exact labels, distinct semantic icons, ordered cue ownership, and stop before CTA, proof, or numeric hard boundaries.
- Portrait presentation specializes seven strongly evidenced structures without changing the shared semantic core: exactly two named inputs use `pairedInputRail`; exactly three equal factors use `factorTrinity`; an asserted cause/driver and target use `causalDriver`; an explicit decisive-factor statement uses `factorPriority`; exactly three ordered workflow steps keep `semanticIntent=workflow-step` and use `compactPipeline`; an asserted capability boundary uses `limitationWarning`; and an explicit prerequisite uses `priorityConclusion`.
- These routes are cardinality- and relation-sensitive. One input is not a pair, two factors are not a trinity, an uncertain cause is not a causal assertion, an ordinary factor mention is not priority, and an incomplete first/then sequence is not a compact pipeline. Route weak or incomplete evidence to the registered fallback.
- `priorityConclusion` is the automatic prerequisite form. `historicalGreenConclusion` is an approved historical visual variant only when the project explicitly sets `presentationVariant=manual-approved`; it is never selected by semantic routing. Green remains completion/success/validation color, so ordinary prerequisites default to blue/gold.
- The format-agnostic guards live in `scripts/semantic_guardrails.py` and are mirrored into every generated project. Landscape and portrait share polarity, future/topic, handoff/prerequisite, ordered-workflow, numeric, proof, explanation, and explicit-viewer-CTA predicates; only their component adapters differ.
...
- Workflow/enumeration, manual-field, capability-share, scene-lock, platform-fanout, asset-variant, and automation-handoff items may only come from caption cues owned by the current source beat. Each step carries an exact-source `label`, exact-source `text`, and non-empty `sourceCueIds`, in source order; require at least two distinct steps except automation handoff, which may use one.
- When the source lacks enough named fields, platforms, capabilities, scenes, ratios, or workflow steps, route to `captionHighlight` with `semanticFallbackFrom` and a specific `fallbackReason`. Do not complete a component with built-in sample data.
...
The regression suite currently covers 123 positive, compound, adversarial, future-preview, numeric-suffix, real-project, short-tail, shared guard, and complete-evidence transformation examples. The shared adapter contract covers 23 cases and the byte-identical shared guardrail suite covers 55 completion, result-evaluation, handoff/prerequisite, ordered-workflow, numeric, topic/future, process/proof, explanation, and CTA cases. `semantic_component_contract_regression.py` additionally checks canonical event types, source-bound short-claim downgrade, same-scene claim selection, the two-claim run limit, no invented platforms/brands/states/ratios, CTA provenance and scheduling priority, approval-gated theme-thesis candidates, transformation provenance/fallback behavior, and schema/renderer type parity. The SFX regression suite covers the confirmed semantic audio suggestions and requires `status: "suggested"`. A route change should update the tests and this document in the same commit.
