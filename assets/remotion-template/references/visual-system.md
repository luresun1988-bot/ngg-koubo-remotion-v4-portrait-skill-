...
- `transformation-stack`: `transformationStack`, for source-proven "from A to B", individual-to-team, moat/leverage, driver-to-result, or productivity shift beats. It uses a top state transition, middle driver chips, and a bottom explicit-result strip. Every visible semantic label must bind through step `text` and `sourceCueIds` to the source beat; gallery defaults, uncited nearby cues, and synthesized results are forbidden. If relation, driver, or result evidence is incomplete, use the audited `captionHighlight` fallback.
...
- `explanation-claim`: use `claimStrip` only for the strongest readable claim in a scene. In portrait fullscreen presenter scenes, place it in the top-safe band above the eyes rather than the old mid-right eye band. Use a small source-bound top-right `statusSticker` when the beat cannot support the full rail, and intentionally render no main HUD for lower-priority repeated claims. Never invent a process diagram or rotate templates only to create variety.
