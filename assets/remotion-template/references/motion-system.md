...
The approved portrait structured components use one deterministic four-phase lifecycle: enter, ordered internal reveal, readable hold, and exit. `pairedInputRail` reveals its two rails in source order; `factorTrinity` staggers three icons; `causalDriver` draws the relation after both endpoints exist; `factorPriority` establishes the supporting factors before emphasizing the decisive one; `compactPipeline` draws exactly three ordered nodes; `limitationWarning` reveals capability before the boundary; and `priorityConclusion`/manual `historicalGreenConclusion` reveal the prerequisite before the conclusion emphasis. All phases must be driven by Remotion frames and remain seek-safe.
