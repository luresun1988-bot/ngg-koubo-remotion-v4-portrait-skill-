...
2. Resolve presenter roles before probing FPS. Prefer repeated `--presenter-video` arguments or `project_config.json.sourceMedia.talkingHeadVideos`; never guess among several ambiguously named videos. Probe the selected primary presenter before building any frame-based timeline. Use its nominal FPS as the composition FPS, normalize fractional/VFR input to that fixed timebase, use 25 fps only when probing is unavailable, and record `presenterSelection` plus `frameRate`.
...
7. Generate or update `06_remotion/visual_script.json` before writing Remotion code. Create real timecoded `captionCues`, run `scripts/split_caption_cues.py` before any beat/event references exist, then run `scripts/semantic_router.py` and `scripts/visual_event_builder.py`. The default split output uses ASCII JSON escapes to avoid Windows shell/codepage corruption.
8. Run `scripts/validate_visual_script.py` immediately after generation. Stop on schema, text-corruption, duplicate cue ID, dangling `sourceCueIds` / `anchorCueId` / `sourceBeatId`, or other hard errors.
...
13. Produce delivery output with `scripts/render_final_and_qa.ps1`. It must run validation, pre-render lint, data generation, TypeScript checking, serial Remotion rendering, BT.709 post-processing, and `final_media_qa.py` as one fail-closed chain. Require matching composition FPS, resolution, decoded frame count, H.264/AAC, `yuv420p`, TV-range BT.709 matrix/transfer/primaries, complete audio coverage, and a successful full-file decode.
...
- Treat fractional rates such as 23.976/29.97/59.94 and detected VFR as normalization inputs, not direct integer-frame timelines. Convert the selected presenter to constant composition FPS while preserving its embedded audio or normalized WAV contract, then require pre-render FPS-contract lint to pass.
- When automatic discovery finds more than one video, accept it only if every filename is an unambiguous ordered presenter-segment name. Otherwise stop and require explicit presenter roles; never concatenate proof recordings or B-roll as presenter footage.
...
- Upgrade an existing `06_remotion` only through `scripts/upgrade_existing_project.py --remotion-root <path> --write`. It may refresh `scripts/` and `references/` with backups, but it must preserve `visual_script.json`, `src/`, `public/`, project config, package files, and all project-specific assets.
...
- Apply the shared semantic guards before portrait component selection. Future previews and ordinary definitions route to `explanation-claim`; episode subject statements route to `topic-intro`; negated handoffs stay negative; process phrases such as `输入关键词生成标题` stay workflow; bare `评论区` / `关键词` / `自提` nouns are not CTA.
- Keep a numeric fact as the primary intent even when completion is negated or unresolved. Add `semanticModifiers: ["numeric", "incomplete"]`, require `negative-incomplete-treatment`, and render a sourced `未完成` state without green completion styling.
- Route unknown or low-confidence ordinary explanation to `explanation-claim`. Use `claimStrip` only for the strongest readable claim in a scene, use a source-bound `statusSticker` for short beats, and record `intentionalCleanHold` for lower-priority repeated claims; never manufacture a workflow diagram merely to keep the screen busy. Route episode/topic introductions to `topic-intro -> topicKeyword`.
...
- Completion is polarity-sensitive. Only asserted results such as "流程已经跑完" or "输出已完成" may use green confirmation; negated, prospective, conditional, partial, noun/meta, future, or unresolved forms must not use `positive-confirm`, green completion styling, or completion SFX.
...
- Generated CTA copy must be source-bound and include `ctaProvenance.sourceText`. CTA routing requires an explicit viewer action such as `评论区回复…`, `私信我`, `关注我`, or `收藏这一条`; bare channel/keyword/pickup nouns and descriptive interaction data are not CTA. Do not invent "评论区", "关键词", "关注", reply actions, or keyword values. If a source CTA would collide with an earlier same-lane HUD, preserve the CTA and trim or drop the earlier HUD while keeping the lane buffer.
...
- Short 3-4 second spoken beats should be merged with neighboring beats when they form one idea. If they cannot be merged, use `timingClass=short-lightweight` plus a source-bound lightweight sticker, or keep a low-confidence repeated explanation intentionally clean; do not force a full long panel into a short sentence.
- Keep at most one low-confidence `claimStrip` per scene and at most two consecutive `claimStrip` main HUDs across scenes. Select the strongest source claim by meaning; mark lower-priority repeats as `intentionalCleanHold` instead of randomly rotating component templates.
...
- In portrait fullscreen/large presenter scenes, keep `claimStrip` in the top-safe band above the eyes. Do not use the old mid-right position that covers the presenter's right eye.
...
For presenter-role, render-chain, FPS-contract, or existing-project upgrade revisions, also run `python scripts/fps_contract_regression.py`, `python scripts/render_pipeline_regression.py`, and `python scripts/upgrade_existing_project_regression.py`.
