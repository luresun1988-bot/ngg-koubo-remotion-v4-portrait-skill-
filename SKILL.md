...
The default suite includes shared semantic-contract checks, a six-case semantic-text → generated visual-script → real Remotion still regression, and a six-keyframe component render smoke. Add `--audit` after dependency changes and `--gallery` after component style/motion changes. Run `scripts/render_motion_previews.ps1` when motion timing, presenter geometry, or lip sync needs review.
