...
The default suite includes shared semantic-contract checks and a six-keyframe real Remotion render smoke. Add `--audit` after dependency changes and `--gallery` after component style/motion changes. Run `scripts/render_motion_previews.ps1` when motion timing, presenter geometry, or lip sync needs review.
