...
- Captions: bottom center, one layer, adaptive dark rounded strip, complete transcript/ASR cue text synchronized to the voice, at most two lines, no ellipsis/truncation/omission, all-white captions; no colored keyword highlights.
...
- Presenter movement: by scene type, using scale and translate easing; no hard jumps.
...
For semantic routing revisions, run `python scripts/semantic_router_regression.py` and `python scripts/sfx_semantic_routing_regression.py`, and require all confirmed spoken/SFX examples to pass before committing.
